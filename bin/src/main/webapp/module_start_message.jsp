...
<body
		...
		<div class="page-container row-fluid">
			...
			<div class="page-content">
				...
				<div class="container-fluid">
					...
					<div class="row-fluid" >
						<div class="span12" >
							<!--           		<button id="addAppBtn"  class="btn green">
                              	<i class="icon-long-arrow-left"></i>返回模块列表
                            </button> -->
							<a href="<c:url value='/app/${appName}'/>" class="btn green">
								<i class="icon-long-arrow-left"></i>返回模块列表
							</a>
	...
	<%-- 	<script src="<%=UI_PATH%>js/moduleStart.js"></script> --%>
	<script>
		jQuery(document).ready(function() {
			var appId = ${appId}
			var mid = ${mid}
			App.init();
			Start.init(appId, mid);
		...
		//启动服务
		var Start = function(appId, mid) {
			return {
				init : function(appId, mid) {
					$.ajax({
						type : "post",
		...
		var Message = function(appId,mid) {
			return {
				init : function(tm,appId,mid) {
					var url = '<c:url value="/module/message"/>'
					if (tm) {
						url += "?tm=" + tm;
					}
					$.ajax({
